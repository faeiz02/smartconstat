...
export class AssurancesComponent implements OnInit {
  ...
  isDeleting = false;
  itemToDelete: Assurance | null = null;
  ...
  applyFilters(): void {
    ...
    if (!q) {
      ...
      this.filteredAssurances = this.assurances.filter(a =>
        (a.assuranceId || '').toLowerCase().includes(q) ||
  ...
  getInitials(a: Assurance): string {
    const n = (a.nom || '?')[0];
    const p = (a.prenom || '?')[0];
  ...
  openAddModal(): void {
    ...
    this.editItem = { assuranceId: '', nom: '', prenom: '', cin: '', phone: '', vehicleBrand: '', vehicleModel: '', vehiclePlate: '', compagnie: '', dateExpiration: '' };
    this.modalMessage = '';
    this.showModal = true;
  }

  openEditModal(item: Assurance): void {
    this.isEditing = true;
    this.editItem = { ...item };
    this.modalMessage = '';
    this.showModal = true;
  ...
  saveItem(): void {
    if (!this.editItem.assuranceId || !this.editItem.cin) {
      this.modalMessage = "L'ID d'assurance et le CIN sont requis.";
    ...
    const obs = this.isEditing
      ? this.clientService.updateAssurance(this.editItem.id, this.editItem)
      : this.clientService.createAssurance(this.editItem);
    ...
    obs.subscribe({
      ...
      error: (err: any) => {
        this.modalMessage = err.error?.error || 'Erreur lors de la sauvegarde';
        this.modalMessageType = 'error';
        this.isSaving = false;
        this.cdr.markForCheck();
      }
  ...
  confirmDelete(): void {
    ...
    this.clientService.deleteAssurance(this.itemToDelete.id).subscribe({
  ...
  // ─── Export CSV ───
  exportCSV(): void {
    const headers = ['ID Assurance', 'Nom', 'Prénom', 'CIN', 'Téléphone', 'Marque', 'Modèle', 'Plaque', 'Compagnie', 'Expiration'];
    const rows = this.assurances.map(a => [
      a.assuranceId, a.nom, a.prenom, a.cin, a.phone,
      a.vehicleBrand, a.vehicleModel, a.vehiclePlate, a.compagnie, a.dateExpiration
    ]);
    const csv = [headers, ...rows].map(r => r.map(v => `"${v || ''}"`).join(',')).join('\n');
    const blob = new Blob(['\ufeff' + csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'assurances.csv'; a.click();
